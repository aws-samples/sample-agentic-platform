"""FastAPI server for the Jira Agent."""
...
from agentic_platform.agent.jira_agent import jira_controller
...
configuration_server_middleware(app, path_prefix="/api/jira-agent")

@app.post("/invocations", response_model=AgenticResponse)
@app.post("/invoke", response_model=AgenticResponse)
# @handle_exceptions(status_code=500, error_prefix="Jira Agent API Error")
async def invoke(request: AgenticRequest) -> AgenticResponse:
    """
    Invoke the Jira agent with a standard response.
    """
    return await jira_controller.invoke(request)


@app.post("/stream")
@handle_exceptions(status_code=500, error_prefix="Jira Agent Streaming API Error")
async def stream(request: AgenticRequest) -> StreamingResponse:
    """
    Stream responses from the Jira agent using Server-Sent Events.
    """
    async def event_generator():
        """Convert StreamEvent objects to SSE format with event names."""
        try:
            async for stream_event in jira_controller.create_stream(request):
